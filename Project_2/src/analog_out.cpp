...
void Analog_out :: set(double duty_cycle){
    if (duty_cycle < 0) duty_cycle = 0; // Keep magnitude to 0 and 255
    if (duty_cycle > 255) duty_cycle = 255;
    timer.set_duty_cycle(duty_cycle);
}

// COMPA fires at TOP (start of each period), COMPB at OCR0B, so the pin is high for OCR0B ticks:
// high-time is proportional to the commanded value.
